...
class RealManSDK:
    ...
    def get_arm_state(self, msg):
        ...
        if self.dx != 0.0 and self.dy != 0.0:
            ...
            inverse_joint = self.inverse_kinematics(predict_pose, current_joint)
    ...
    def forward_main(self):
        ...
        self.pub_canfd = rospy.Publisher('/rm_driver/MoveJ_Fd_Custom_Cmdd', JointPosCustom, queue_size=20)
        self.sub = rospy.Subscriber('/rm_driver/Arm_Current_State', Arm_Current_State, self.get_arm_state)
        time.sleep(1)
        ...
        try:
            ...
            while (not sdk.ans_return() and count < 30) or count < 5:
                # 获取视频帧
                            ...
                            if label in str("meter,digital_meter_right,digital_meter_left,digital_meter_mid,red_floatmeter,iron_floatmeter,ph_meter") and conf > 0.65:
                                ...
                                if abs(d_x) > tolerance or abs(d_y) > tolerance:
                                    ...
                                    try:
                                        ...
                                        cmd = GetArmState_Command()
                                        cmd.command = 'get_current_arm_state'
                                        self.pub_get_state.publish(cmd)
                                        time.sleep(0.2)
...
if __name__ == "__main__":
    ...
    # arm.rm_movej([0,0,0,0,0,0],20,0,0,1)
    # time.sleep(0.5)
    ...
    euler = [current_pose[3], current_pose[4], current_pose[5]]
    print(euler)

    q_sdk = sdk.euler2quot(euler)
    print(q_sdk, 'q of sdk')
    q_arm = arm.rm_algo_euler2quaternion(euler)
    print(q_arm, 'q of arm')

    print('equal or not: ', np.allclose(q_arm, q_sdk))
    print('difference: ', [q_sdk[i] - q_arm[i] for i in range(4)])
